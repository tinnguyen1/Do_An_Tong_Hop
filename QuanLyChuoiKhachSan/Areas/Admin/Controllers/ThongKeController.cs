...
namespace QLKS.Areas.Admin.Controllers.Admin
{
    public class ThongKeController : Controller
    {
        ...
        private void Tab()
        {
            ...
            var tong = db.ChiTietHoaDons.Where(t => t.NgayKetThuc >= firstDayOfMonth && t.NgayKetThuc <= lastDayOfMonth).Sum(t => t.Gia);
            if (tong != null)
                ViewBag.tien_ht = String.Format("{0:0,0.00}", tong);
            else
                ViewBag.tien_ht = "0";

            ViewBag.so_hoa_don = db.ChiTietHoaDons.Count();
            //ViewBag.so_phieu_dat_phong = db.CT_DatPhong.Where(u => u.ma_tinh_trang == 1).Count();
            //ViewBag.so_phong_dang_dat = db.PhongKhachSans.Where(u => u.ma_tinh_trang == 2).Count();
        ...
        private void Char1(DateTime start, DateTime end)
        {
            ...
            for (int i = 0; i <= num; i++)
            {
                ...
                var q = db.ChiTietHoaDons.Where(t => t.NgayKetThuc > f1 && t.NgayKetThuc < f2).Sum(t => t.Gia);
                if (q == null)
                    q = 0;
                tong += (double)q;
                C1sl.Add((Double)q);
            ...
            ViewBag.Gia = "Tổng doanh thu từ ngày " + start.ToShortDateString() + " tới ngày " + end.ToShortDateString() + " là " + String.Format("{0:0,0.00}", tong) + " VND";
            ViewBag.C1sl = C1sl;
        ...
        private void Char2(DateTime end)
        {
            ...
            for (int i = 1; i <= 7; i++)
            {
                ...
                DateTime f2 = f1.AddDays(1);
                var q = db.ChiTietHoaDons.Where(t => t.NgayBatDau >= f1 && t.NgayBatDau < f2).Count();
                C2sl.Add(q);
        ...
        private void Char3()
        {
            var s = db.HoaDon_DichVu.GroupBy(t => t.DichVu.TenDichVu).Select(t => new { TenDichVu = t.Key, total = t.Sum(i => i.MaPhong) });
            List<String> name = new List<String>();
            List<int> total = new List<int>();
            foreach (var group in s)
            {
                System.Diagnostics.Debug.WriteLine("Ma Dv: " + group.TenDichVu + " | SL: " + group.total);
                name.Add((String)group.TenDichVu);
